 ...
 * @param event event
 */
export const handler: FirehoseTransformationHandler = async (event: FirehoseTransformationEvent) => {
 ...
 * @returns 
 */
function processRecord(record: FirehoseTransformationEventRecord): FirehoseTransformationResultRecord {
  try {
    const ddbStreamData = JSON.parse(Buffer.from(record.data, 'base64').toString());

    const result: ddbHistory = {
